...
travis_fold start git.checkout
  if [[ ! -d jobbix/tic-tac-toe/.git ]]; then
    travis_cmd git\ clone\ --depth\=50\ --branch\='master'\ https://github.com/jobbix/tic-tac-toe.git\ jobbix/tic-tac-toe --assert --echo --retry --timing
  else
    travis_cmd git\ -C\ jobbix/tic-tac-toe\ fetch\ origin --assert --echo --retry --timing
    travis_cmd git\ -C\ jobbix/tic-tac-toe\ reset\ --hard --assert --echo
  fi
  travis_cmd cd\ jobbix/tic-tac-toe --echo
...
export TRAVIS_BUILD_DIR=$HOME/build/jobbix/tic-tac-toe
export TRAVIS_JOB_ID=''
export TRAVIS_JOB_NUMBER=''
export TRAVIS_BRANCH=''
export TRAVIS_COMMIT=''
export TRAVIS_COMMIT_RANGE=''
export TRAVIS_REPO_SLUG=jobbix/tic-tac-toe
...
travis_cmd ant\ jar --echo --timing
travis_result $?
